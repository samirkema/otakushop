...
export async function GET(request: Request) {
  ...
  const { data, error, count } = await (svc as any)
    .from('tableaux')
    .select('id, title, artist, thumbnail, price_eur, available, created_at', { count: 'exact' })
...
export async function POST(request: Request) {
  ...
  const { data, error } = await (svc as any)
    ...
    .insert({
      ...
      price_eur:   b.price_eur != null ? Number(b.price_eur) : null,
      price_btc:   b.price_btc != null ? Number(b.price_btc) : null,
      available:   b.available !== false,
      created_by:  guard.userId,
    })
    .select('id, title, artist, thumbnail, available')
    .single();
